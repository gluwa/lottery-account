...
// File @openzeppelin/contracts/cryptography/ECDSA.sol@v2.4.0
...
// File contracts/abstracts/EnumerableSet.sol
...
// File contracts/abstracts/AccessControl.sol
...
contract AccessControlUpgradeSafe is Initializable, Context {
    ...
    modifier onlyAdmin() {
        require(
            hasRole(_roles[DEFAULT_ADMIN_ROLE].adminRole, _msgSender()),
            "AccessControl: Sender do not have admin right"
        );
        _;
    }
    ...
    ETHlessTransfer
{
    ...
    constructor(
        ...
        string memory symbol,
        uint8 decimals
    ) public ExtendedERC20(name, symbol, decimals) {
    ...
    function _collect(address sender, uint256 amount) internal {
        _transfer(sender, _feeCollector, amount);
    }
